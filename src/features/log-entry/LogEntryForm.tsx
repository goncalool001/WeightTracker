...
export function LogEntryForm() {
  ...
  // Prefill the weight field with the most recent value as a sensible default.
  useEffect(() => {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (Number.isNaN(value) || value < WEIGHT_MIN || value > WEIGHT_MAX) {
      toast.error(`Enter a weight between ${WEIGHT_MIN} and ${WEIGHT_MAX} kg.`);
      return;
    }
    const action = addOrUpdate(date, value);
    toast.success(
      `${action === 'added' ? 'Added' : 'Updated'} ${formatDate(date)} — ${value.toFixed(1)} kg`,
    );
  }

  return (
    <form onSubmit={onSubmit} className="card themed p-4">
      <h2 className="mb-3 text-sm font-bold text-text">Log weight</h2>

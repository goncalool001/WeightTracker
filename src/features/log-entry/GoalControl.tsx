...
export function GoalControl() {
  ...
  // Keep the field in sync if the goal changes elsewhere (e.g. import).
  useEffect(() => {
    ...
    <div className="card themed p-4">
      ...
      <p className="mt-2 min-h-[1rem] text-xs text-muted">
        {goalWeight != null && projection
          ? `On track to reach ${goalWeight.toFixed(1)} kg around ${formatDate(projection.date)}.`
          : goalWeight != null
            ? 'Shown as a line on the charts.'
            : 'Set a target to see a goal line and projection.'}
      </p>
    </div>
  );
}
